...
const ModalView: FC<IProps> = ({ open, setOpen }) => {
  ...
  return (
    <>
      {open.item && (
        <Modal title={'Xem'} open={open.active} onCancel={handleCancel} okButtonProps={{ style: { display: 'none' } }} cancelText='Đóng' okText={false}>
          <Divider />
          <div className='flex flex-col gap-4 text-base'>
            <Card>
              <div className='flex items-center'>
                <p className='mr-2 inline-block text-base font-medium'>Họ tên:</p>
                <p>
                  <Avatar.Group className='flex items-center'>
                    {open.item?.avatar && <img className='mr-4 size-7 object-cover' src={open.item.avatar}></img>}
            ...
            {open.item?.information && (
              <Card>
                ...
                <div style={{ whiteSpace: 'pre-line' }}>{open.item.information}</div>
              </Card>
            )}
          </div>
        </Modal>
